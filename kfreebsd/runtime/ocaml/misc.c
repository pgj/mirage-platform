...
{
  __fprintf (stderr, "file %s; line %d ### Assertion failed: %s\n",
           file, line, expr);
#if defined(__FreeBSD__) && defined(_KERNEL)
  printf("caml_failed_assert");
#else
  fflush (stderr);
  exit (100);
...
{
  __fprintf (stderr, "%s", msg);
#if defined(__FreeBSD__) && defined(_KERNEL)
  printf("caml_fatal_error");
#else
  exit(2);
#endif
}
...
CAMLexport void caml_fatal_error_arg2 (char *fmt1, char *arg1,
                                       char *fmt2, char *arg2)
{
  __fprintf (stderr, fmt1, arg1);
  __fprintf (stderr, fmt2, arg2);
#if defined(__FreeBSD__) && defined(_KERNEL)
...
CAMLexport value Val_some(value v) {
  CAMLparam1(v);
  CAMLlocal1(some);
  some = caml_alloc(1, 0);
