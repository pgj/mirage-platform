...
CAMLprim value caml_parse_engine(struct parser_tables *tables, struct parser_env *env, value cmd, value arg);
                                 ...
                                 struct parser_env *env, value cmd, value arg)
{
  ...
  recover:
    if (errflag < 3) {
      ...
      while (1) {
        ...
        if (n1 != 0 && n2 >= 0 && n2 <= Int_val(tables->tablesize) &&
          ...
          if (caml_parser_trace)
            __fprintf(stderr, "Recovering in state %d\n", state1);
          goto shift_recover;
        } else {
          if (caml_parser_trace){
            __fprintf(stderr, "Discarding state %d\n", state1);
          }
          if (sp <= Int_val(env->stackbase)) {
            if (caml_parser_trace){
              __fprintf(stderr, "No more states to discard\n");
            }
      ...
      if (Int_val(env->curr_char) == 0)
        return RAISE_PARSE_ERROR; /* The ML code raises Parse_error */
      if (caml_parser_trace)
          __fprintf(stderr, "Discarding last token read\n");
  ...
  shift_recover:
    if (caml_parser_trace)
      __fprintf(stderr, "State %d: shift to state %d\n",
  ...
  reduce:
    if (caml_parser_trace)
      __fprintf(stderr, "State %d: reduce by rule %d\n", state, n);
