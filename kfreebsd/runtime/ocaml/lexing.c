...
CAMLprim value caml_lex_engine(struct lexing_table *tbl, value start_state, struct lexer_buffer *lexbuf);
CAMLprim value caml_new_lex_engine(struct lexing_table *tbl, value start_state, struct lexer_buffer *lexbuf);
...
static void run_mem(char *pc, value mem, value curr_pos) {
  for (;;) {
    ...
    src = *pc++ ;
    if (src == 0xff) {
      /*      __fprintf(stderr,"[%hhu] <- %d\n",dst,Int_val(curr_pos)) ;*/
      Field(mem,dst) = curr_pos ;
    } else {
      /*      __fprintf(stderr,"[%hhu] <- [%hhu]\n",dst,src) ; */
